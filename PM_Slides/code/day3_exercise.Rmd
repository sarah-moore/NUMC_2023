...
```{r}
wvs_ideology <- wvs5 %>% select(COW, V114, V116, V204)
```
...
```{r}
wvs_ideology <- wvs_ideology %>% rename(country_code = COW,
                                          ideol_scale = V114,
                                          income_equal = V116,
...
```{r}
wvs_ideology <- wvs_ideology %>% filter(country_code  %in% c(2,140,551,740, 380,850))
```
...
```{r}
wvs_ideology <- wvs_ideology %>% mutate(country_name = case_when(
  country_code == 2 ~ "U.S.",
  country_code == 140 ~ "Brazil",
  country_code == 551 ~ "Zambia",
  country_code == 380 ~ "Sweden",
  country_code == 740 ~ "Japan",
  country_code == 850 ~ "Indonesia"
))
```
...
```{r}
wvs_ideology <- wvs_ideology %>% mutate(ideol_scale = ifelse(ideol_scale < 0, NA, ideol_scale))
```
...
Now that we know we have complete cases, let's begin by getting a simple average of ideology in the left-right scale.

```{r}
wvs_ideology %>% summarize(mean_ideol = mean(ideol_scale))
```
If there are NAs in the dataset, we need to include a specification to the code above:

```{r}
wvs_ideology %>% summarize(mean_ideol = mean(ideol_scale, na.rm = TRUE))
```
Now get the average left-right scale for each country. Hint:  use `group_by`


```{r}
wvs_ideology %>% group_by(country_name) %>% summarize(mean_lr = mean(ideol_scale, na.rm=TRUE))
```
...
```{r}
wvs_ideology %>% group_by(country_name) %>% summarize(mean_lr = mean(ideol_scale, na.rm=TRUE),
                                                      mean_income = mean(income_equal, na.rm=TRUE),
                                                      mean_abort = mean(abort_justified, na.rm=TRUE))
```
