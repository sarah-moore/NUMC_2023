...
```{r, include = T, echo = F, message=F, out.width= "35%", fig.align="center"}
library(tidyverse)

ggplot() + 
  stat_function(fun=function(x) (x^2)) + 
   geom_segment(aes(x = 1, y = 4, xend = 1.84, yend = 4),
                  arrow = arrow(length = unit(0.5, "cm")), color = "red") + 
   geom_segment(aes(xend = 2.15, yend = 4, x = 3, y = 4),
                  arrow = arrow(length = unit(0.5, "cm")), color = "red") + 
  xlim(-1, 4)
...
```{r, include = T, echo = F, message=F, out.width= "35%", fig.align="center", warning=F}
library(tidyverse)

ggplot() + 
  stat_function(fun=function(x) ((2*x^3))) + 
  stat_function(fun=function(x) (6*x^2), linetype = 2, color = "blue") + 
  theme_minimal() +
  ylim(-2,2) + 
  xlim(-2, 2)
...
.pull-left[
```{r, include = T, echo = F, message=F,  fig.align="center", warning=F}
ggplot() + 
  stat_function(fun=function(x) ((x^3)), xlim = c(-10, 10)) + 
  theme_minimal() +
  xlim(-10,10) + 
  labs(title = "YES", x= "", y = "") + 
  theme(plot.title = element_text(color = "green", face = "bold", size = 16))
```
]
...
ggplot() + 
  stat_function(fun=function(x) ((x^3)), xlim = c(-10, -0.25)) + 
  geom_point(aes(x = -0.25, y = -0.25^3), size = 2.5) + 
  stat_function(fun=function(x) ((x^3)), xlim = c(0.25, 10)) + 
    geom_point(aes(x = .25, y = 0.25^3), size = 2.5) + 
  theme_minimal() +
  xlim(-10,10) + 
  labs(title = "NO", x= "", y = "") + 
  theme(plot.title = element_text(color = "red", face = "bold", size = 16))
