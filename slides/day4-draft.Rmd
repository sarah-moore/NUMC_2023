...
- Let's say a new book store opens and based on the first 100 days of sales we can model daily book sales given the function $f(x)= ln(x) * 20$, where $x$ is the number of days since the bookstore opened. 

- As we know, solving for $x$ will give us the amount of books sold on that particular day. 

```{r, include = T, echo = F, message=F,  fig.align="center", warning=F, out.width= "45%"}
library(tidyverse)

ggplot() + 
    stat_function(fun = function(x) (log(x)*20), 
                xlim = c(0,100)) + 
...
ggplot() + 
  ...
  ggtitle("Daily Book Sales, Days 0-100")+ 
  ylab("Number of Books Sold")+ 
  xlab("Days Since Store Opening")->book_sales_area
...
```{r, include = T, echo = F, message=F,  fig.align="center", warning=F,  out.width= "55%"}
library(patchwork)
dta <- tibble(x = 25:65) %>% 
  mutate(y = log(x)*20)

book_sales + 
  geom_col(data = dta, aes(x, y), width = 1, fill = "gray", alpha = 0.5, color = "black") -> intervals1

book_sales + 
  geom_col(data = dta, aes(x, y), width = 0.1, fill = "gray", alpha = 0.5, color = "black") -> intervals2

intervals1 + intervals2 + plot_layout(nrow = 1)
?plot_layout
```
...
>- $F(x) = \frac{3}{2}x^2 + 3x +c$

--

- $\int 16x^2 + 2x + 7 dx$ 

>- $F(x)= \frac{16}{3}x^3 + x^2 + 7x + c$

--

- $\int \sqrt{x} + 13 dx$

>- $F(x)= \frac{2}{3}x^{\frac{3}{2}} + 13x + c$
...
>- $F(x)= \frac{x^4}{4}-\frac{x^3}{3}+x^2+c$
...
>- $-\frac{1}{2x^2}|^5_1=0.48$

--

3. $\int^{1}_0 e^x dx$ **hint: the integral of $e^x$ is the same as its derivative.**

>- $e^x|^1_0 = (e-1) \hspace{0.5cm} \text{or} \hspace{0.5cm} 1.7183$
